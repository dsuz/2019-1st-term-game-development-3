...
using UnityEngine;
using System.Threading.Tasks;   // async/await を使うために必要
using UnityEngine.UI;

[RequireComponent(typeof(AudioSource))]
public class CountdownControllerA : MonoBehaviour
{
    ...
    /// <returns></returns>
    async void Count(int count)
    {
        for (int i = count; i > -1; i--)
        {
            await Task.Delay(System.TimeSpan.FromSeconds(m_countdownInterval)); // 待つ
